...
// fetch scattering factor from scattering_texture (single or multiple), 
...
// return the single scattering or the n-th order of scattering
...
RadianceDensitySpectrum ComputeScatteringDensity(IN(AtmosphereParameters) atmosphere,
  ...
  vec3 omega = vec3(sqrt(1.0 - mu * mu), 0.0, mu); // view dir in XoZ plane
  Number sun_dir_x = omega.x == 0.0 ? 0.0 : (nu - mu * mu_s) / omega.x; // 解三棱锥顶角
  Number sun_dir_y = sqrt(max(1.0 - sun_dir_x * sun_dir_x - mu_s * mu_s, 0.0));
  vec3 omega_s = vec3(sun_dir_x, sun_dir_y, mu_s); // [cos(phi)sin(mu_s), sin(phi)sin(mu_s), cos(mu_s)]
  ...
  for (int l = 0; l < SAMPLE_COUNT; ++l) {
    ...
    for (int m = 0; m < 2 * SAMPLE_COUNT; ++m) {
      ...
      // the sum of a term given by the precomputed scattering texture for the (n-1)-th order:
      ...
      // contribution n-1 bounces and whose last bounce(n-2) is on the ground
      ...
      // final radiance  scattered from direction omega_i towards direction -omega
...
// compute the n-th order of scattering, using scattering_density_texture
